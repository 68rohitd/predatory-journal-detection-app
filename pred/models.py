...
class Journaltable(models.Model):
    ...
    Using_bogus_metric_and_index = models.CharField(
        max_length=200, choices=Metric[2])
...
class Journaltable2(models.Model):
    ...
    author_fee_for_subscription_model = models.CharField(
        max_length=200, choices=Metric[8])
    author_fee_for_open_access = models.CharField(
        max_length=200, choices=Metric[9])
    charging_for_readers_for_open_access = models.CharField(
        max_length=200, choices=Metric[10])
    journal_name = models.CharField(max_length=200, choices=Metric[11])
    conflicts_of_interest = models.CharField(
        max_length=200, choices=Metric[12])
    ...
    clarity_of_abstract_Internation_Journal = models.CharField(
        max_length=200, choices=Metric[20])
    clarity_of_abstract_Regional_Journal = models.CharField(
        max_length=200, choices=Metric[21])
    quality_of_and_comformity_with_stated_aim = models.CharField(
        max_length=200, choices=Metric[22])
    readability_of_article = models.CharField(
        max_length=200, choices=Metric[23])
    content_availability = models.CharField(max_length=200, choices=Metric[24])
    english_language_homepage_availability_International_journal = models.CharField(
        max_length=200, choices=Metric[25])
    regional_language_homepage_availability_Regional_journal = models.CharField(
        max_length=200, choices=Metric[26])
    If_open_access_is_it_listed_in_directory_of_open_access_journal = models.CharField(
        max_length=200, choices=Metric[27])
    affiliation_of_editors = models.CharField(
        max_length=200, choices=Metric[28])
    diversity_in_geographical_distribution_of_editors = models.CharField(
        max_length=200, choices=Metric[29])
    diversity_in_geographical_distribution_of_authors = models.CharField(
        max_length=200, choices=Metric[30])
    plagiarism = models.CharField(max_length=200, choices=Metric[31])
    similarity_of_website_design = models.CharField(
        max_length=200, choices=Metric[32])
    editors_standing = models.CharField(max_length=200, choices=Metric[33])
    convincing_editorial_scope = models.CharField(
        max_length=200, choices=Metric[34])
    send_jornal_spam_email_to_review_papers = models.CharField(
        max_length=200, choices=Metric[35])
    questionable_special_issue = models.CharField(
        max_length=200, choices=Metric[36])
    Predatory_1_Non_predatory_0 = models.CharField(
        max_length=200, choices=Metric[37])
...
class Parameter(models.Model):
    ...
    def __str__(self):
        # removed parameter_name before {0} for better formatting...
        return " {0}".format(self.parameter_name)
...
class Journal(models.Model):
    ...
    def __str__(self):
        # removed journal_name before {0}
        return " {0}".format(self.journal_name)
...
class Options(models.Model):
    ...
    def __str__(self):
        # removed option_name for {0} for better formatting...
        return " {0} {1} SCORE {2}".format(self.parameter, self.option_name, self.score)


class JournalDatabase(models.Model):
    journal = models.ForeignKey(Journal, on_delete=models.CASCADE)
    parameter = models.ForeignKey(Parameter, on_delete=models.CASCADE)
    chosen_option = models.ForeignKey(Options, on_delete=models.CASCADE)

    def __str__(self):
        return "journal {0} parameter{1} chosen_option {2}".format(self.journal, self.parameter, self.chosen_option)
